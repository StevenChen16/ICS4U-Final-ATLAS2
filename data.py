...
def calculate_connors_rsi(data, rsi_period=3, streak_period=2, rank_period=100):
    ...
    price_rsi = pd.Series(
        ta.RSI(data["Close"].values, timeperiod=rsi_period), index=data.index
    )
    ...
    streak_rsi = pd.Series(
        ta.RSI(streak_values, timeperiod=streak_period), index=data.index
    )

    # Component 3: Percentage Rank (ROC)
    def percent_rank(series, period):
        return series.rolling(period).apply(
            lambda x: pd.Series(x).rank().iloc[-1] / float(period) * 100, raw=True
        )
...
def apply_kalman_filter(data, measurement_noise=0.1, process_noise=0.01):
    ...
    return pd.Series(filtered_prices, index=data.index), pd.Series(
        trends, index=data.index
    )
...
def download_and_prepare_data(symbol, start_date, end_date, window=21):
    ...
    if isinstance(stock.columns, pd.MultiIndex):
        stock = (
            stock.xs(symbol, level=1, axis=1)
            if symbol in stock.columns.levels[1]
            else stock
        )
    ...
    stock["MACD"], stock["MACD_Signal"], stock["MACD_Hist"] = ta.MACD(
        stock["Close"].values
    )

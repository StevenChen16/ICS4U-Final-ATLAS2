...
def get_multiple_stocks_data(symbols, period="1d", interval="1m"):
    ...
    for i, symbol in enumerate(symbols):
        ...
        if df is not None:
            all_data[symbol] = df
            # Update cache
            data_cache[symbol] = {"df": df, "timestamp": datetime.now()}
...
def calculate_simple_prediction(df):
    ...
    latest_signal = (
        df["MACD_Signal"].dropna().iloc[-1]
        if not df["MACD_Signal"].dropna().empty
        else 0
    )
    latest_crsi = df["CRSI"].dropna().iloc[-1] if not df["CRSI"].dropna().empty else 50
    latest_kalman_trend = (
        df["Kalman_Trend"].dropna().iloc[-1]
        if not df["Kalman_Trend"].dropna().empty
        else 0
    )

    # Price trend (5-day)
    price_trend = (
        df["Price_Change_5"].dropna().iloc[-1]
        if not df["Price_Change_5"].dropna().empty
        else 0
    )
    ...
    volume_ratio = (
        ...
        if not pd.isna(df["Volume_MA"].iloc[-1])
        else 1
    )
    ...
    bb_upper = (
        df["BB_Upper"].dropna().iloc[-1]
        if not df["BB_Upper"].dropna().empty
        else latest_close
    )
    bb_lower = (
        df["BB_Lower"].dropna().iloc[-1]
        if not df["BB_Lower"].dropna().empty
        else latest_close
    )
    bb_position = (
        (latest_close - bb_lower) / (bb_upper - bb_lower)
                    ...
                    dbc.Row(
                        [
                            dbc.Col(
                                [
                                    ...
                                    dcc.Dropdown(
                                        ...
                                        options=[
                                            {
                                                "label": f"{symbol} - {get_stock_name(symbol)}",
                                                "value": symbol,
                                            }
                            ...
                            dbc.Col(
                                [
                                    html.Label(
                                        "Time Period:", className="fw-bold mb-2"
                                    ),
                                    dcc.Dropdown(
                    ...
                    [
                        dbc.Card(
                            [
                                ...
                                dbc.CardBody([html.Div(id="predictions-summary")]),
                            ]
                        )
                    ],
                    width=4,
                ...
                dbc.Row(
                    [
                        ...
                        dbc.Col(
                            [
                                html.P(
                                    "⚡ Powered by YFinance + Plotly Dash",
...
@app.callback(
    [Output("multi-stock-data-store", "data"), Output("last-update", "children")],
    [
...
def update_multi_stock_data(n_intervals, n_clicks, selected_symbols, timeframe):
    ...
    for symbol, df in all_stock_data.items():
        if df is not None and not df.empty:
            ...
            processed_data[symbol] = {
                ...
                "change": (
                    float(df["Close"].iloc[-1] - df["Close"].iloc[-2])
                    if len(df) > 1
                    else 0
                ),
                "change_pct": (
                    float((df["Close"].iloc[-1] / df["Close"].iloc[-2] - 1) * 100)
                    if len(df) > 1
                    else 0
                ),
                "volume": (
                    int(df["Volume"].iloc[-1])
                    if not pd.isna(df["Volume"].iloc[-1])
                    else 0
                ),
                "rsi": (
    ...
    print(
        f"✅ Successfully fetched {success_count}/{len(selected_symbols)} stocks data"
    )
...
@app.callback(
    Output("stock-overview-cards", "children"), Input("multi-stock-data-store", "data")
)
def update_stock_overview_cards(data):
    """Update stock overview cards"""
    if not data or "stocks" not in data:
        return html.Div(
            "Select stocks to view data", className="text-center text-muted p-4"
        )
    ...
    for symbol, stock_data in stocks.items():
        ...
        card = dbc.Col(
                    ...
                    [
                        dbc.CardBody(
                            [
                                ...
                                html.H4(
                                    f"${price:.2f}", className=f"mb-1 {price_color}"
                                ),
                                html.P(
            ...
            width=12 // min(len(stocks), 6),
        )  # Auto-responsive column width
...
@app.callback(
    ...
    [Input("multi-stock-data-store", "data"), Input("indicator-selector", "value")],
)
def update_price_comparison_chart(data, indicators):
    ...
    fig = make_subplots(
        ...
        subplot_titles=["Multi-Stock Price Comparison"]
        + (["Volume Comparison"] if "VOL" in indicators else []),
    )
...
@app.callback(
    Output("predictions-summary", "children"), Input("multi-stock-data-store", "data")
)
def update_predictions_summary(data):
    """Update prediction summary"""
    if not data or "stocks" not in data:
        return html.Div(
            "No prediction data available", className="text-center text-muted"
        )
        ...
        [
            dbc.CardBody(
                [
                    ...
                    html.P(
                        f"Bullish: {buy_signals}/{total_stocks}",
                        className="small mb-1 text-success",
                    ),
                    html.P(
                        f"Bearish: {sell_signals}/{total_stocks}",
                        className="small mb-1 text-danger",
                    ),
                    html.P(
                        f"观望: {total_stocks-buy_signals-sell_signals}/{total_stocks}",
...
def update_detailed_analysis(data, indicators):
    ...
    if "RSI" in indicators:
        ...
        rsi_fig.update_layout(
            template="plotly_dark", height=300, yaxis=dict(range=[0, 100])
        )
    ...
    if "CRSI" in indicators:
        ...
        crsi_fig.add_hline(
            y=80, line_dash="dash", line_color="red", annotation_text="Overbought(80)"
        )
        crsi_fig.add_hline(
            y=20, line_dash="dash", line_color="green", annotation_text="Oversold(20)"
        )
        crsi_fig.update_layout(
            template="plotly_dark", height=300, yaxis=dict(range=[0, 100])
        )
...
if __name__ == "__main__":
    ...
    print(
        "🔧 Technical indicators: MA, Bollinger Bands, RSI, MACD, Connors RSI, Kalman Filter, FFT Filter"
    )
